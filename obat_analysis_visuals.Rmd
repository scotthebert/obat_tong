...
```{r}
agerace.famp45 <- glm(`Family planning missing` ~ Age, family = "binomial", 
                    data = obat45)
summary(agerace.famp45)
exp(summary(agerace.famp45)$coefficients[,1])
...
summary(fpfu)
exp(summary(fpfu)$coefficients[,1])
```
...
obat45v2 <- obat45
obat45v2$`Family planning missing` <- relevel(as.factor(obat45v2$`Family planning missing`), ref = "1")
obat45v2$`Pap smear missing` <- relevel(as.factor(obat45v2$`Pap smear missing`), ref = "1")


fpps <- glm(`Family planning missing` ~ `Pap smear missing`, family = "binomial", 
            data = obat45v2)
summary(fpps)
exp(summary(fpps)$coefficients[,1])
```
